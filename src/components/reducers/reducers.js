import { ADD_TO_CART, FILTER_CATEGORY } from '../actions/actionTypes'
...
const reducer = (state = initState, action) => {
    switch (action.type) {
        case ADD_TO_CART:
            let addedItem = state.items.find(item => item.imdbID === action.id);
            let exitedItem = state.addedItems.find(item => item.imdbID === action.id);
            if (exitedItem) {
