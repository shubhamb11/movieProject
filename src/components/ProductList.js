...
class ProductList extends Component {
    ...
    render() {
        ...
        let cardList = this.props.items.map(item => {
            return (
                <Col md={4} xs={12} sm={6} lg={3} key={item.imdbID} id={item.imdbID}>
        ...
        return (
            <React.Fragment>
                ...
                }} variant="info" onClose={() => this.setAlert(false)} show={this.state.showAlert === true} delay={2000} autohide>
                    <Toast.Header>
                        Added to Cart!
                    </Toast.Header>
                </Toast>
...
const mapStateToProps = (state) => {
    if (state.categorySelected === "all") {
        ...
        return {
            items: state.items.filter(item => item.Type === state.categorySelected)
        }
    }
